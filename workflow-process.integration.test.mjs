...
function legacyPlanningState(input) { const value = state.createState(input); value.stage = "implementing"; value.stageSequence = [...state.LEGACY_STAGE_SEQUENCE.filter(stage => stage !== "planning")]; value.history = [{ stage: "planning", at: value.createdAt }, { stage: "implementing", at: value.updatedAt }]; return value; }
...
test("workflow cache expires cross-process changes while transactions remain authoritative", async () => {
  ...
  try {
    ...
    assert.equal((await cache.get(workflowId, state.loadState)).stage, "implementing");
    await child("workflow-cache-update-fixture.mjs", [workflowId, "testing"]);
    assert.equal((await cache.get(workflowId, state.loadState)).stage, "implementing", "fresh TTL may serve a local snapshot");
    assert.equal((await state.transactState(workflowId, current => current.stage)).result, "testing", "transaction bypasses cache and observes the other process immediately");
    now = 250;
    assert.equal((await cache.get(workflowId, state.loadState)).stage, "testing", "expired cache reloads durable cross-process state");
...
test("workflow subprocess receives raw identity and transactions cannot resurrect a terminal state", async () => {
  ...
  try {
    ...
    const output = await child("workflow-child-fixture.mjs", [workflowId, "advance-testing"], {
      PI_SUBAGENT_CHILD: "1", PI_SUBAGENT_ID: "workflow-ticket_child_42:implementer",
