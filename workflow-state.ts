...
export interface WorkflowState {
  version: typeof CURRENT_STATE_VERSION; id: string; goal: string; tickets: string[]; acceptanceCriteria: string[]; repositoryRoot: string; stage: Stage;
...
export function migrateState(raw: unknown): WorkflowState {
  ...
  const state: WorkflowState = {
    ...candidate as Omit<WorkflowState, "version" | "id" | "goal" | "tickets" | "acceptanceCriteria" | "repositoryRoot" | "stage" | "history" | "agentHandles" | "filesOwned" | "tests" | "review" | "followUps" | "systemOfRecord" | "template" | "stageSequence" | "createdAt" | "updatedAt">,
    version: CURRENT_STATE_VERSION,
    id: string(candidate.id, "id"), goal: string(candidate.goal, "goal"), tickets: (() => { const values = stringArray(candidate.tickets ?? [candidate.goal], "tickets").map(item => item.trim()).filter(Boolean); if (!values.length) fail("tickets must contain at least one non-empty ticket"); return values; })(), acceptanceCriteria: stringArray(candidate.acceptanceCriteria, "acceptanceCriteria"), repositoryRoot: string(candidate.repositoryRoot, "repositoryRoot"), stage: stage(candidate.stage, "stage"),
...
export function createState(input: { id: string; goal: string; tickets?: string[]; acceptanceCriteria?: string[]; repositoryRoot: string; systemOfRecord?: WorkflowState["systemOfRecord"]; mode?: Exclude<WorkflowMode, "recovery">; inheritedEvidence?: InheritedEvidence[]; acceptedDeviations?: PolicyDecision[] }): WorkflowState {
  ...
  const tickets = (input.tickets ?? [input.goal]).map(item => item.trim()).filter(Boolean);
  if (!tickets.length) throw new Error("At least one non-empty ticket is required");
  return { version: CURRENT_STATE_VERSION, id: input.id, goal: input.goal, tickets, acceptanceCriteria: [...(input.acceptanceCriteria ?? [])], repositoryRoot: path.resolve(input.repositoryRoot), stage: "red_testing",
